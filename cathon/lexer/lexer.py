...
class Lexer(object):
  ...
  def parse(self) -> list[Token]:
    ...
    while self.char:
      ...
      if self.char == '#' or (self.char == '/' and self.lookahead() == '/'):
        token = self.make_inline_comment()
      elif self.char == '/' and self.lookahead() == '*':
        token = self.make_block_comment()
      elif self.char == '\\':
        token = Token(NL, pos_start=self.pos)
        self.advance()
      elif (DIGITS(self.char) and self.char != '.') or (
        self.char == '.'
        and self.lookahead()
        and DIGITS(self.lookahead())
        and self.lookahead() != '.'
      ):
        ...
        raise errors.SyntaxError(
          pos_start,
          self.pos.copy(),
          f'invalid character {repr(char)} (U+{hex(ord(char))[2:].zfill(4)})',
        )
      res.append(token)
      if brackets_nl:
  ...
  def make_inline_comment(self):
    ...
    while self.char and self.char != '\n':
      ...
      self.advance()
    return Token(COMMENT, ''.join(res), pos_start, self.pos.copy())
  ...
  def make_block_comment(self):
    ...
    self.advance()
    while self.char:
      if self.char == '*' and self.lookahead() == '/':
        break
      res.append(self.char)
      self.advance()

    if self.char != '*' or self.lookahead() != '/':
      raise errors.SyntaxError(pos_start, self.pos, 'expected "*/"')
    self.advance(2)
    return Token(COMMENT, ''.join(res), pos_start, self.pos.copy())
    ...
    while self.char and self.char in (' ', '\t'):
      if self.indent_type is None:
        ...
        raise errors.TabError(
          self.pos, self.pos, 'inconsistent use of tabs and spaces in indentation'
        )
      count += 1
      self.advance()
    res = []
  ...
  def make_num(self, base=10):
    ...
    pos_start = self.pos.copy()
    is_float = False
    while self.char and (LETTERS_DIGITS(self.char) or self.char in ['_', '.']):
      if self.char == '.':
        is_float = True
      if self.char == '_':
        self.advance()
        ...
        self.advance()
        raise errors.SyntaxError(
          pos_start, self.pos, f'invalid {bases[base][0]} literal'
        )
  ...
  def make_string(self, quotation) -> Token:
    ...
    self.advance()
    while (
      ((quotation not in ('“', '”')) and self.char != quotation)
      or (quotation in ('“', '”') and self.char not in ('“', '”'))
    ) or escape_character:
      if self.char is None or self.char == '\n':
        raise errors.SyntaxError(
          self.pos,
          self.pos,
          f'unterminated string literal (expected {repr(quotation)} )',
        )
      if escape_character:
