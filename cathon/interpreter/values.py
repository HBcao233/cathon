...
def cat_getattr(object, attr, default=None):
  if not isinstance(object, (Object, Object._CAT__class__)):
    return default
...
class Object:
  ...
  def invalid(self, op, other=None, details=None):
    if details is None:
      details = (
        f'invalid {"unary" if other is None else "binary"} operation: {OP_REDICT[op]}'
      )
...
class Type(Object):
  ...
  def CAT__dict__(self):
    ...
    return mappingproxy(
      {k[3:]: v for k, v in self.__class__.__dict__.items() if k.startswith('CAT')}
    )
  ...
  def __getitem__(self, key):
    if not isinstance(key, int) and key.name not in ('int', 'slice'):
      raise errors.TypeError(
        self.pos_start,
        self.pos_end,
        f'string indices must be integers or slices, not {key.name}',
        self.context,
      )
    try:
  ...
  def CAT__getitem__(self, key):
    if not isinstance(key, (Int, Slice)):
      raise errors.TypeError(
        self.pos_start,
        self.pos_end,
        f'list indices must be integers or slices, not {key.name}',
        self.context,
      )
    try:
...
class Slice(Object):
  ...
  def get_object(self):
    return slice(
      self.start.get_object(), self.stop.get_object(), self.step.get_object()
    )
  ...
  def CAT__call__(self, *args, **kwargs):
    try:
      ...
      raise errors.TypeError(
        self.pos_start,
        self.pos_end,
        f"bad operand type for {name.replace('_', '')}(): '{self.CAT__name__}'",
        self.context,
      )
